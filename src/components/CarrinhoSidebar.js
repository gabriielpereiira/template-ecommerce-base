  ...
  async function handleBuscarFrete(cepParam) {
    ...
    try {
      ...
      if (res.ok && json?.data) {
        ...
        setFreteErro(json?.erro || 'CEP nao atendido para entrega')
        setFreteData(null)
      }
    } catch (e) {
    ...
    ;(async () => {
      try {
        ...
        if (result?.success && result.data?.init_point) {
          ...
          setToast(result?.erro || 'Erro ao iniciar pagamento')
          setFinalizando(false)
        }
      } catch (e) {
  ...
  return (
    <>
      ...
      <div style={{
        ...
        width: '100%', maxWidth: '420px', background: COLORS.white,
        ...
        <div style={{
          ...
          <h2 style={{ margin: 0, fontSize: '20px', fontWeight: 700, color: COLORS.dark, fontFamily: SERIF }}>
            Sacola{totalItens > 0 ? ' (' + totalItens + ')' : ''}
          </h2>
          <button onClick={handleFechar} style={{
            background: 'none', border: 'none', cursor: 'pointer',
            color: COLORS.textSecondary, padding: '4px', display: 'flex',
          }}>
        ...
        <div style={{ borderTop: '1px solid ' + COLORS.border, padding: '20px 24px' }}>
          ...
          <div style={{ marginBottom: '16px' }}>
            ...
            <div style={{ display: 'flex', gap: '8px' }}>
              <input type="text" value={cep}
                ...
                style={{
                  ...
                  border: '1px solid ' + (freteExigido && !freteData ? '#D32F2F' : COLORS.border),
                  fontSize: '14px', fontFamily: SANS, color: COLORS.dark,
                  outline: 'none', boxSizing: 'border-box',
                }}
              />
              <button onClick={() => handleBuscarFrete()} disabled={freteCarregando} style={{
            ...
            {freteCarregando && <p style={{ margin: '4px 0', fontSize: '12px', color: COLORS.textSecondary }}>Calculando frete...</p>}
            {freteErro && <p style={{ margin: '4px 0', fontSize: '12px', color: '#D32F2F' }}>{freteErro}</p>}
            {freteExigido && !freteData && !freteCarregando && (
              <p style={{ margin: '4px 0', fontSize: '12px', color: '#D32F2F' }}>
                Informe um CEP e clique em Calcular antes de finalizar.
              </p>
            )}
            {freteData && (
              <div style={{
                padding: '10px 12px', background: '#E8F5E9', borderRadius: '8px',
                marginTop: '8px', border: '1px solid #C8E6C9',
              }}>
          ...
          {!cupomData ? (
            <div style={{ marginBottom: '16px' }}>
              ...
              <div style={{ display: 'flex', gap: '8px' }}>
                ...
                />
                <button onClick={handleAplicarCupom} disabled={cupomCarregando} style={{
                  padding: '10px 16px', borderRadius: '8px', border: 'none',
              ...
              {cupomErro && <p style={{ margin: '4px 0', fontSize: '12px', color: '#D32F2F' }}>{cupomErro}</p>}
            </div>
          ) : (
            <div style={{
              marginBottom: '16px', padding: '10px 12px', background: '#E8F5E9',
              ...
              <div>
                ...
                <p style={{ margin: '2px 0 0', fontSize: '12px', color: '#388E3C' }}>
                  {cupomData.tipo === 'percentual' ? `${cupomData.valor}% de desconto` : `${formatarPreco(cupomData.valor)} de desconto`}
                </p>
              </div>
              <button onClick={handleRemoverCupom} style={{
                background: 'none', border: 'none', color: '#D32F2F',
                cursor: 'pointer', fontSize: '13px', fontWeight: 600, fontFamily: SANS, padding: '4px 8px',
              }}>
                Remover
              </button>
            </div>
          ...
          {/* Botao finalizar */}
          <button onClick={handleFinalizar} disabled={finalizando || !freteData} style={{
            ...
            fontFamily: SANS, letterSpacing: '0.5px', textTransform: 'uppercase',
            opacity: finalizando ? 0.7 : 1, marginBottom: '10px',
            transition: 'all 0.3s ease',
          }}>
            {finalizando ? 'Redirecionando...' : 'Finalizar Pedido'}
          </button>

          <button onClick={limparCarrinho} disabled={finalizando} style={{
            ...
            fontFamily: SANS, opacity: finalizando ? 0.5 : 1,
          }}>
            Limpar sacola
          </button>
        </div>
      ...
      {toast && (
        <div style={{
          ...
          fontFamily: SANS, boxShadow: '0 4px 16px rgba(45,27,14,0.2)',
